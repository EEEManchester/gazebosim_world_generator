...
class Environment(object):
	...
	def read_params(self, f_name):
		...
		if "output_radiation_filename" in data[self._prefix[:-1]]:
			self._radiation_file = data[self._prefix[:-1]]["output_radiation_filename"]
		if "output_config_file_name" in data[self._prefix[:-1]]:
			self._output_config_file_name = data[self._prefix[:-1]]["output_config_file_name"]
		if "templates_folder" in data[self._prefix[:-1]]:
			self._templates_folder = data[self._prefix[:-1]]["templates_folder"]
		...
		if "metres_per_pixel" in data[self._prefix[:-1]]:
			self._metres_per_pixel = data[self._prefix[:-1]]["metres_per_pixel"]
		if "barrel_pos_noise" in data[self._prefix[:-1]]:
			self._barrel_pos_noise = data[self._prefix[:-1]]["barrel_pos_noise"]
		if "barrel_stacking_noise" in data[self._prefix[:-1]]:
			self._barrel_stacking_noise = data[self._prefix[:-1]]["barrel_stacking_noise"]
		...
		if "radiation_scaling" in data[self._prefix[:-1]]:
			self._radiation_scaling = data[self._prefix[:-1]]["radiation_scaling"]
		if "radiation_units_alpha" in data[self._prefix[:-1]]:
			self._radiation_units_alpha = data[self._prefix[:-1]]["radiation_units_alpha"]
		if "radiation_units_beta" in data[self._prefix[:-1]]:
			self._radiation_units_beta = data[self._prefix[:-1]]["radiation_units_beta"]
		if "radiation_units_gamma" in data[self._prefix[:-1]]:
			self._radiation_units_gamma = data[self._prefix[:-1]]["radiation_units_gamma"]
		if "radiation_output_file_format" in data[self._prefix[:-1]]:
			self._radiation_output_file_format = data[self._prefix[:-1]]["radiation_output_file_format"]
	...
	def generate(self):
		...
		if not os.path.exists(self._outputfile[:-1*len(self._outputfile.split("/")[-1])]):
			os.makedirs(self._outputfile[:-1*len(self._outputfile.split("/")[-1])])
						...
						for c in self._custom_models:
							...
							if (col[0] == array[x_index][y_index][0]) & (col[1] == array[x_index][y_index][1]) & (col[2] == array[x_index][y_index][2]):
								...
								for i in range(0,stacking):
									...
									keywords['pose'] = str(self.array_to_map(x_index, "x")+noise_x) + " " + str(self.array_to_map(y_index, "y")+noise_y) + " " + str(self._custom_models[c]["z_offset"] + (height*(i))) + " 0 -0 " + str(self._custom_models[c]["rotation"]+noise_yaw)
