...
class Environment(object):
    ...
    """ DEPRICATED FOR RADIATION SOURCES/SENSORS WORK"""
    ...
    # 					if (a[0] > 0):
    # 						sources["source_"+str(count)] = {'x': x, 'y': y, 'z': 0, 'value': float(
    # 							a[0]*self._radiation_scaling), 'type': 'alpha', 'units': str(self._radiation_units_alpha), 'noise': 2.0}
    # 						count += 1
    # 					if (a[1] > 0):
    # 						sources["source_"+str(count)] = {'x': x, 'y': y, 'z': 0, 'value': float(
    # 							a[1]*self._radiation_scaling), 'type': 'beta', 'units': str(self._radiation_units_beta), 'noise': 2.0}
    # 						count += 1
    # 					if (a[2] > 0):
    # 						sources["source_"+str(count)] = {'x': x, 'y': y, 'z': 0, 'value': float(
    # 							a[2]*self._radiation_scaling), 'type': 'gamma', 'units': str(self._radiation_units_gamma), 'noise': 2.0}
    # 						count += 1
    # 				elif self._radiation_output_file_format == 1:
    # 					if (a[0] > 0):
    # 						sources["source_"+str(count)] = {'x': x}, {'y': y}, {'z': 0}, {'value': float(a[0]*self._radiation_scaling)}, {
    # 							'type': 'alpha'}, {'units': str(self._radiation_units_alpha)}, {'noise': 2.0}
    # 						count += 1
    # 					if (a[1] > 0):
    # 						sources["source_"+str(count)] = {'x': x}, {'y': y}, {'z': 0}, {'value': float(a[1]*self._radiation_scaling)}, {
    # 							'type': 'beta'}, {'units': str(self._radiation_units_beta)}, {'noise': 2.0}
    # 						count += 1
    # 					if (a[2] > 0):
    # 						sources["source_"+str(count)] = {'x': x}, {'y': y}, {'z': 0}, {'value': float(a[2]*self._radiation_scaling)}, {
    # 							'type': 'gamma'}, {'units': str(self._radiation_units_gamma)}, {'noise': 2.0}
    # 						count += 1
    ...
    def gen_objects(self):
        ...
        leftovers = (blue_no_white-self._barrels) + \
            (red_no_white-self._blocks) + self._size_array
    ...
    def check_param_server(self, prefix):
        ...
        elif not hasattr(self, '_input_map_name'):
        ...
        elif not hasattr(self, '_output_map_name'):
        ...
        elif not hasattr(self, '_outputfile'):
            self._outputfile = "default_value"
        if rospy.has_param(prefix+"output_radiation_filename"):
            self._radiation_file = rospy.get_param(
                prefix+"output_radiation_filename")
        if rospy.has_param(prefix+"output_config_file_name"):
            self._output_config_file_name = rospy.get_param(
                prefix+"output_config_file_name")
        elif not hasattr(self, '_output_config_file_name'):
        ...
        elif not hasattr(self, '_metres_per_pixel'):
        ...
        elif not hasattr(self, '_barrel_pos_noise'):
            self._barrel_pos_noise = "default_value"
        if rospy.has_param(prefix+"barrel_stacking_noise"):
            self._barrel_stacking_noise = rospy.get_param(
                prefix+"barrel_stacking_noise")
        elif not hasattr(self, '_barrel_stacking_noise'):
        ...
        elif not hasattr(self, '_object_heights'):
            self._object_heights = "default_value"
        if rospy.has_param(prefix+"radiation_scaling"):
            self._radiation_scaling = rospy.get_param(
                prefix+"radiation_scaling")
        elif not hasattr(self, '_radiation_scaling'):
            self._radiation_scaling = "default_value"
        """
        if rospy.has_param(prefix+"radiation_units_alpha"):
            self._radiation_units_alpha = rospy.get_param(
                prefix+"radiation_units_alpha")
        ...
        if rospy.has_param(prefix+"radiation_units_beta"):
            self._radiation_units_beta = rospy.get_param(
                prefix+"radiation_units_beta")
        ...
        if rospy.has_param(prefix+"radiation_units_gamma"):
            self._radiation_units_gamma = rospy.get_param(
                prefix+"radiation_units_gamma")
        ...
        if rospy.has_param(prefix+"radiation_output_file_format"):
            self._radiation_output_file_format = rospy.get_param(
                prefix+"radiation_output_file_format")
        ...
        elif not hasattr(self, '_custom_models'):
    ...
    def read_params(self, f_name):
        ...
        if ".json" in f_name:
            ...
            data = {}
        if "input_map" in data[self._prefix[:-1]]:
            self._input_map_name = data[self._prefix[:-1]]["input_map"]
        if "output_map" in data[self._prefix[:-1]]:
            self._output_map_name = data[self._prefix[:-1]]["output_map"]
        if "radiation_map" in data[self._prefix[:-1]]:
            self._input_rad_map_name = data[self._prefix[:-1]]["radiation_map"]
        if "output_filename" in data[self._prefix[:-1]]:
            self._outputfile = data[self._prefix[:-1]]["output_filename"]
        if "output_radiation_filename" in data[self._prefix[:-1]]:
            self._radiation_file = data[self._prefix[:-1]
                                        ]["output_radiation_filename"]
        if "output_config_file_name" in data[self._prefix[:-1]]:
            self._output_config_file_name = data[self._prefix[:-1]
                                                 ]["output_config_file_name"]
        if "templates_folder" in data[self._prefix[:-1]]:
            self._templates_folder = data[self._prefix[:-1]
                                          ]["templates_folder"]
        if "box" in data[self._prefix[:-1]]:
            self._box = data[self._prefix[:-1]]["box"]
        if "cylinder" in data[self._prefix[:-1]]:
            self._cylinder = data[self._prefix[:-1]]["cylinder"]
        if "walls" in data[self._prefix[:-1]]:
            self._wall_height = data[self._prefix[:-1]]["walls"]
        if "map_centre" in data[self._prefix[:-1]]:
            self._map_centre = data[self._prefix[:-1]]["map_centre"]
        if "sizing" in data[self._prefix[:-1]]:
            self._sizing = data[self._prefix[:-1]]["sizing"]
        if "metres_per_pixel" in data[self._prefix[:-1]]:
            self._metres_per_pixel = data[self._prefix[:-1]
                                          ]["metres_per_pixel"]
        if "barrel_pos_noise" in data[self._prefix[:-1]]:
            self._barrel_pos_noise = data[self._prefix[:-1]
                                          ]["barrel_pos_noise"]
        if "barrel_stacking_noise" in data[self._prefix[:-1]]:
            self._barrel_stacking_noise = data[self._prefix[:-1]
                                               ]["barrel_stacking_noise"]
        if "heights" in data[self._prefix[:-1]]:
            self._object_heights = data[self._prefix[:-1]]["heights"]
        if "radiation_scaling" in data[self._prefix[:-1]]:
            self._radiation_scaling = data[self._prefix[:-1]
                                           ]["radiation_scaling"]
        if "radiation_units_alpha" in data[self._prefix[:-1]]:
            self._radiation_units_alpha = data[self._prefix[:-1]
                                               ]["radiation_units_alpha"]
        if "radiation_units_beta" in data[self._prefix[:-1]]:
            self._radiation_units_beta = data[self._prefix[:-1]
                                              ]["radiation_units_beta"]
        if "radiation_units_gamma" in data[self._prefix[:-1]]:
            self._radiation_units_gamma = data[self._prefix[:-1]
                                               ]["radiation_units_gamma"]
        if "radiation_output_file_format" in data[self._prefix[:-1]]:
            self._radiation_output_file_format = data[self._prefix[:-1]
                                                      ]["radiation_output_file_format"]
        if "custom_models" in data[self._prefix[:-1]]:
            self._custom_models = data[self._prefix[:-1]]["custom_models"]
    ...
    def generate(self):
        ...
        #print self._outputfile[:-1*len(self._outputfile.split("/")[-1])]
        if not os.path.exists(self._outputfile[:-1*len(self._outputfile.split("/")[-1])]):
            os.makedirs(
                self._outputfile[:-1*len(self._outputfile.split("/")[-1])])

        open(self._outputfile, "w").writelines(
            [l for l in open(self._templates_folder+"headerfile.sdf").readlines()])
        ...
        open(self._outputfile, "a").writelines(
            [l for l in open(self._templates_folder+"ground_plane.sdf").readlines()])

        open(self._outputfile, "a").writelines(
            [l for l in open(self._templates_folder+"endfile.sdf").readlines()])
    ...
    def place_walls(self, walls, f_name):
        ...
        for i in walls:
            ...
            pose = str(self.array_to_map(centre_pos_x, "x")) + " " + str(self.array_to_map(
                centre_pos_y, "y")) + " " + str(float(self._wall_height["height"])/2.0) + " 0 0 0"
            size = str(width*self._metres_per_pixel) + " " + str(length *
                                                                 self._metres_per_pixel) + " " + str(self._wall_height["height"])
            colour = str(self._wall_height["colour"])
            keywords = {'name': name, 'pose': pose,
    ...
    def populate(self, array, f_name):
        ...
        for x_index in range(0, array.shape[0]):
            for y_index in range(0, array.shape[1]):
                ...
                if test > 1:
                    keywords = {'name': '', 'pose': '', 'radius': '',
                                            'length': '', 'radius': '', 'size': '', 'colour': ''}
                    if f_name == "custom":
                        for c in self._custom_models:
                            col = np.asarray(
                                (self._custom_models[c]["value"].lstrip().split(" ")), dtype=int)
                            if (col[0] == array[x_index][y_index][0]) & (col[1] == array[x_index][y_index][1]) & (col[2] == array[x_index][y_index][2]):
                                if ("stacking" in self._custom_models[c].keys()) & ("height" in self._custom_models[c].keys()):
                                    stacking = self._custom_models[c]["stacking"]
                                    height = self._custom_models[c]["height"]
                                else:
                                    stacking = 1
                                    height = 1

                                #print self._custom_models[c]["name"],stacking , height

                                for i in range(0, stacking):
                                    name = self._custom_models[c]["name"]
                                    if ("pose_noise" in self._custom_models[c].keys()):
                                        n = self._custom_models[c]["pose_noise"]
                                    else:
                                        n = 0.0
                                    if ("rotation" in self._custom_models[c].keys()):
                                        rotation = self._custom_models[c]["rotation"]
                                    else:
                                        rotation = 0
                                    noise_x = self.noise(n)
                                    noise_y = self.noise(n)
                                    noise_yaw = self.noise(n)
                                    keywords['name'] = name+"_"+str(count)
                                    keywords['pose'] = str(self.array_to_map(x_index, "x")+noise_x) + " " + str(self.array_to_map(y_index, "y")+noise_y) + " " + str(
                                        self._custom_models[c]["z_offset"] + (height*(i))) + " 0 -0 " + str(rotation+noise_yaw)
                                    if ("dae_location" in self._custom_models[c].keys()):
                                        keywords['dae_location'] = self._custom_models[c]["dae_location"]
                                    else:
                                        keywords['dae_location'] = ""
                                    #print keywords
                                    ...
                                    if name == "source":
                                        ...
                                        file_name = f_name
                                    open(self._outputfile, "a")
                                    with open(self._templates_folder+"{}.sdf".format(file_name)) as f:
                                        with open(self._outputfile, "a") as f1:
                                            for line in f:
                                                f1.write(line.format(
                                                    '', **keywords))
                                    count += 1
                        ...
                        if f_name == "cylinder":
                            ...
                            name = "cylinder"
                            keywords['radius'] = self._cylinder["r"] * \
                                self._metres_per_pixel
                            keywords['length'] = self._cylinder["z"] * \
                                self._metres_per_pixel
                        elif f_name == 'box':
                            name = "box"
                            if self._size_array[x_index][y_index] == 0:
                                keywords['size'] = str(self._box["x"]*self._metres_per_pixel) + " " + str(
                                    self._box["y"]*self._metres_per_pixel) + " " + str(self._box["z"]*self._metres_per_pixel)
                            else:
                                keywords['size'] = str(self._sizing[str(self._size_array[x_index][y_index])]*self._metres_per_pixel) + " " + str(self._sizing[str(
                                    self._size_array[x_index][y_index])]*self._metres_per_pixel) + " " + str(self._sizing[str(self._size_array[x_index][y_index])]*self._metres_per_pixel)

                        #print self._prefix[:-1]
                        #print self._json_dict
                        #print self._json_dict[self._prefix[:-1]]
                        if self._json_dict[self._prefix[:-1]][name]["colour"]:
                            keywords['colour'] = self._json_dict[self._prefix[:-1]
                                                                 ][name]["colour"]

                        try:
                            stack = self._object_heights[str(
                        ...
                        for z_index in range(0, stack):
                            keywords['name'] = name+"_"+str(count)
                            if name == "cylinder":
                                keywords['pose'] = str(self.array_to_map(x_index, "x") + noise_x + self.noise(self._barrel_stacking_noise)) + " " + str(self.array_to_map(y_index, "y") + noise_y + self.noise(self._barrel_stacking_noise)) + " " + str(
                                    ((self._json_dict[self._prefix[:-1]][f_name]["z"]*z_index)+(self._json_dict[self._prefix[:-1]][f_name]["z"])/2.0)*self._metres_per_pixel*self._sizing[str(self._size_array[x_index][y_index])]) + " 0 -0 0"
                            elif name == 'box':
                                keywords['pose'] = str(self.array_to_map(x_index, "x")) + " " + str(self.array_to_map(y_index, "y")) + " " + str(((self._json_dict[self._prefix[:-1]][f_name]["z"]*z_index)+(
                                    self._json_dict[self._prefix[:-1]][f_name]["z"])/2.0)*self._metres_per_pixel*self._sizing[str(self._size_array[x_index][y_index])]) + " 0 -0 0"
